...
const GrassStepEffect = preload("res://grass_step_effect.tscn")
...
func _ready():
	get_tree().current_scene.find_child("Character").connect("player_moving_signal", player_exiting_grass)
	get_tree().current_scene.find_child("Character").connect("player_stopped_signal", player_in_grass)
